...
run_simulation_random <- function(
  ...
  if (t_stat >= threshold_null) {
    theta_cons <- c(alpha1_hat[1:3], alpha2_hat[1:3])
  } else {
    res <- alabama::auglag(
      par    = c(alpha1_hat[1:3], alpha2_hat[1:3]),
      fn     = joint_loglik,
      hin    = const_positive,
      heq    = const_distance,
      control.outer = list(trace = FALSE)
  ...
  for (b in seq_len(B)) {
    ...
    dat1b <- simulate_competing_risks(
      ...
      beta      = c(0, 0, 0),
      cov_prob  = 0.5,
      censoring = list(model = "exp", param = alpha1_hat[4])
    )
  ...
  # Step 5: Decision
  # -------------------------------------------------------------
  crit_val <- quantile(boot_vals, alpha, type = 1, names = FALSE)
  reject   <- ifelse(t_stat < crit_val, 1, 0)
  return(reject)
}
